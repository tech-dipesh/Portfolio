...
export default function Projectcart({image, Project}) {
  return (
 <article className='flex flex-col lg:flex-row gap-6 bg-slate-900/50 p-4 md:p-8 rounded-2xl border border-slate-800 hover:border-slate-700 transition-all duration-300 group w-full min-w-0'>
      <div className='flex-1 min-w-0 relative'>
        <div className='bg-slate-800/50 p-4 md:p-6 rounded-xl border border-slate-700/50 h-full overflow-hidden'>
          <p className="text-slate-400 text-sm md:text-lg mb-4 md:mb-6 leading-relaxed">
            {Project.intro}
          </p>
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-5 relative">
        ...
        <div className='pt-4 pr-10'>
          <div className='flex items-center gap-3 mb-2'>
            ...
            <h2 className="text-xl md:text-3xl font-bold text-slate-100 leading-tight">
              {Project.name}
            </h2>
          </div>
        ...
        <div className='mt-auto pt-6 border-t border-slate-800'>
          ...
          <div className='flex flex-wrap gap-2 md:gap-3'>
            {Project.techStack.map((tech, index) => (
              <div 
                key={index} 
                className="px-3 md:px-4 py-1.5 md:py-2 bg-slate-800 border border-slate-700 rounded-lg text-slate-300 text-xs md:text-sm hover:border-blue-500/50 hover:bg-slate-700 transition-all duration-300 cursor-default"
              >
                {tech}
              </div>
            ))}
